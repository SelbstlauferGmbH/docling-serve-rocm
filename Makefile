...
docling-serve-cu130-image: Dockerfile ## Build docling-serve container image with CUDA 13.0 support
	$(ECHO_PREFIX) printf "  %-12s Dockerfile\n" "[docling-serve with Cuda 13.0]"
	$(CMD_PREFIX) $(CONTAINER_RUNTIME) build --load --build-arg "UV_SYNC_EXTRA_ARGS=--no-group pypi --group cu130" -f Dockerfile --platform linux/amd64 -t ghcr.io/docling-project/docling-serve-cu130:$(TAG) .
